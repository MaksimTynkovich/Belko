...
function replaceSpanText(inputString) {
    // Создаем временный div элемент для парсинга HTML
    let tempDiv = document.createElement('div');
    tempDiv.innerHTML = inputString;

    // Находим все span элементы с классом 'testik'
    let spans = tempDiv.querySelectorAll('span.time');

    // Проходимся по каждому найденному span элементу и меняем текст
    spans.forEach(span => {
        if (span.textContent.trim() === '1 seconds ago') {
            span.textContent = '1 секунду назад';
        }
    });
...
function convertTimeAgoToRussian(agoString) {
    // Разделяем строку на число и единицу измерения времени
    const parts = agoString.split(' ');
    const number = parseInt(parts[0]); // Получаем число
    const unit = parts[1]; // Получаем единицу измерения времени

    // Определяем нужное окончание для слова "минута"
    let minuteWord;
    if (number === 1) {
        minuteWord = 'минуту';
    ...
    // Составляем и возвращаем строку в нужном формате
    return `${number} ${minuteWord} назад`;
}
  ...
  if (messenger != 0 && !noMoreMessages && !messagesLoading) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        if (messagesPage == 1) {
          // if(data.messages.indexOf("Say 'hi' and start messaging")){
          //     messagesElement.html("<p class=\"message-hint center-el\"><span>Начните переписку</span></p>");
          // }else{
              messagesElement.html(data.messages);

            let spans = document.querySelectorAll('.message-hint span');

            spans.forEach(span => {
                // Получаем текст внутри элемента
                let text = span.textContent || span.innerText;

                // Например, проверяем, если текст равен 'example'
                if (text === 'Say \'hi\' and start messaging') {
                    span.innerHTML = "Начните общение"
                }else if(text === 'Nothing shared yet'){
                    span.innerHTML = "Нет вложений"
                }else if(text === "Type to search.."){
                    span.innerHTML = "Введите для поиска..."
                }
            });

            let times = document.querySelectorAll('span.time');
            times.forEach(time => {
                time.innerHTML = convertTimeAgoToRussian(time.innerHTML)
            })



            // $(document).ready(function(){
            // console.log($('p.message-hint span').text() + "111")
            //     if($('div.messages-container div.messages p.message-hint span').text() == "'Say 'hi' and start messaging'"){
            //         console.log("tt")
            //     }
            //
            // });
